...
if (isset($_POST['uname']) && isset($_POST['password'])){
    ...
    $sql = "SELECT * FROM schedule_student s INNER JOIN access_token a ON a.exam_id=s.exam_id
    INNER JOIN exam e ON e.exam_id=s.exam_id
              WHERE reg_no='".$name."' AND s.exam_id='".$test."' 
                 AND ((token='$pass' AND  user_id=stdid) OR (token='$pass' AND  a.user_id IS NULL) )
               limit 1";
    $query = mysqli_query(conn(), $sql) or die(mysqli_error(conn()));

    // echo mysqli_num_rows($query);return;
