...
require_once 'Admin/asset/classes/db.php';
 	...
 	$row = mysqli_fetch_assoc($query);
 	 $testname = $row['name']."";
    $filename = str_replace(" ", "_", $testname)."Result_".date('Y')."(Batch_".$batch.")";         //File Name
	...
	if ($batch === 'all') {
		...
		$Q = 'SELECT schedule_student.surname,schedule_student.othername,schedule_student.dept,
				schedule_student.reg_no,test.name,testscore.right_answered,
		...
		$Q = 'SELECT schedule_student.surname,schedule_student.othername,schedule_student.dept,
				...
				INNER JOIN test ON test.test_id = schedule_student.test_id
				JOIN track_timer as tr ON tr.stdid = schedule_student.stdid 
				WHERE testscore.testid = '.$test_id.' AND batch="'.$batch.'" AND test.test_id = "'.$test_id.'" ORDER BY reg_no';
	}
